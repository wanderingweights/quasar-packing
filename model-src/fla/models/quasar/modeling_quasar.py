...
from fla.models.quasar.configuration_quasar import QuasarConfig
from fla.models.utils import Cache, FLAGenerationMixin
from fla.modules import FusedCrossEntropyLoss, FusedLinearCrossEntropyLoss, RMSNorm
from fla.modules import GatedMLP as QuasarMLP
...
class QuasarBlock(GradientCheckpointingLayer):
...
class QuasarPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        ...
        if isinstance(module, QuasarAttention) and next(module.parameters()).device.type != "meta":
            with torch.no_grad():
...
class QuasarModel(QuasarPreTrainedModel):
    def __init__(self, config: QuasarConfig):
        ...
        self.layers = nn.ModuleList([QuasarBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
