...
@triton.heuristics({
    ...
    'USE_QUASAR_ALPHA': lambda args: args['beta_out'] is not None,
})
@triton.autotune(
    configs=[
...
def chunk_quasar_fwd_kernel_intra_token_parallel(
    q,
    k,
    g,
    beta,
    ...
    # 1. QUASAR CT ALGORITHM
    b_k2 = tl.sum(b_k * b_k, axis=1)
    b_k2_clamped = tl.where(b_k2 < 0.1, 0.1, tl.where(b_k2 > 10.0, 10.0, b_k2))
    b_alpha = (1.0 - tl.exp(-b_beta * b_k2_clamped)) / (b_k2_clamped + 1e-6)
...
def chunk_quasar_fwd_intra_token_parallel(
    q: torch.Tensor,
    k: torch.Tensor,
    gk: torch.Tensor,
    beta: torch.Tensor,
    ...
    Args:
        ...
        beta_out: Required output tensor for Quasar CT scalar decay
    """
    B, T, H, K = q.shape
    ...
    chunk_quasar_fwd_kernel_intra_token_parallel[grid](
        q=q,
        k=k,
        g=gk,
        beta=beta,
