    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        for i_v in range(tl.cdiv(V, BV)):
            ...
            if i_k == 0:
                ...
                b_v = tl.load(p_v, boundary_check=(0, 1))
                b_dv_inner = tl.load(p_dv, boundary_check=(0, 1)) # Re-load to break dependency across debug_barrier

                b_dA += tl.dot(b_dv_inner.to(b_v.dtype), tl.trans(b_v))

                b_dvb = tl.dot(b_A, b_dv_inner.to(b_A.dtype))
...
def chunk_quasar_bwd_wy_dqkb_fused(
    ...
    # H200/B200 Triton autotune can benchmark invalid BK/BV/warps candidates for
    # this Quasar backward and leave the CUDA context in illegal-address state.
    # Force a conservative tile that fits head_dim=128 without autotune.
